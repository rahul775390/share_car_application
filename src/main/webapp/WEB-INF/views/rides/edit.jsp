...
<head>
    ...
    <title>Edit Ride - CarSharing</title>
    <!-- Bootstrap 5 CSS -->
...
<body>
    ...
    <main class="container form-container mb-5">
        <div class="bg-white p-4 border rounded-4 shadow-sm">
            <h3 class="fw-bold mb-1">Edit Ride</h3>
            <p class="text-muted mb-4">Update your published route schedules.</p>
            ...
            <form action="${pageContext.request.contextPath}/rides/edit" method="POST" class="needs-validation" novalidate>
                <input type="hidden" name="id" value="${ride.id}">
                
                <div class="row g-3">
                    <div class="col-md-6">
                        <label for="source" class="form-label fw-semibold">Source City</label>
                        <input type="text" class="form-control" id="source" name="source" value="${ride.source}" required>
                        <div class="invalid-feedback">Source location is required.</div>
                    </div>
                    <div class="col-md-6">
                        <label for="destination" class="form-label fw-semibold">Destination City</label>
                        <input type="text" class="form-control" id="destination" name="destination" value="${ride.destination}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="date" name="date" value="${ride.rideDate}" required>
                        <div class="invalid-feedback">Please select departure date.</div>
                    </div>
                    <div class="col-md-6">
                        <label for="time" class="form-label fw-semibold">Departure Time</label>
                        <input type="time" class="form-control" id="time" name="time" value="${ride.rideTime}" required>
                        <div class="invalid-feedback">Please select departure time.</div>
                    </div>
                    <div class="col-12">
                        <label for="description" class="form-label fw-semibold">Ride Details</label>
                        <textarea class="form-control" id="description" name="description" rows="3">${ride.description}</textarea>
                    </div>
                </div>

                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="btn btn-primary-custom flex-grow-1">Save Changes</button>
                    <a href="${pageContext.request.contextPath}/rides/details?id=${ride.id}" class="btn btn-outline-secondary px-4">Cancel</a>
                </div>
            </form>
        </div>
    </main>
