...
<head>
    ...
    <title>My Offered Rides - CarSharing</title>
    <!-- Bootstrap 5 CSS -->
...
<body>
                ...
                <div class="d-flex align-items-center">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 mt-2" aria-labelledby="profileDropdown">
                            ...
                            <li><a class="dropdown-item active" href="${pageContext.request.contextPath}/rides"><i class="bi bi-compass me-2"></i>My Offered Rides</a></li>
    ...
    <!-- Main Container -->
    <main class="container rides-container mb-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold mb-1">My Offered Rides</h2>
                <p class="text-muted mb-0">Publish routes and share travels with other passengers.</p>
            </div>
            <a href="${pageContext.request.contextPath}/rides/create" class="btn btn-primary-custom">
                <i class="bi bi-plus-circle me-2"></i>Publish a Ride
            </a>
        </div>
        ...
        <c:if test="${param.added == 'true'}">
            <div class="alert alert-success alert-dismissible fade show" role="alert">
                <i class="bi bi-check-circle-fill me-2"></i>Ride created successfully!
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        </c:if>
        <c:if test="${param.updated == 'true'}">
            <div class="alert alert-success alert-dismissible fade show" role="alert">
                <i class="bi bi-check-circle-fill me-2"></i>Ride details updated successfully!
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        </c:if>
        <c:if test="${param.cancelled == 'true'}">
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i>Ride has been cancelled.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        </c:if>
        ...
        <c:choose>
            <c:when test="${empty rides}">
                <div class="bg-white p-5 border rounded-4 shadow-sm text-center my-5">
                    <div class="display-1 text-muted mb-3"><i class="bi bi-compass"></i></div>
                    <h4 class="fw-bold">No rides published yet</h4>
                    <p class="text-muted">Fill in source and destination coordinates to share seats and split commuting costs.</p>
                    <a href="${pageContext.request.contextPath}/rides/create" class="btn btn-primary-custom mt-2">Publish Your First Ride</a>
                </div>
            </c:when>
            <c:otherwise>
                <div class="row g-4">
                            ...
                            <div class="card border rounded-4 shadow-sm h-100 bg-white">
                                <div class="card-body p-4 d-flex flex-column justify-content-between">
                                    <div>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-borderless table-sm mb-3 align-middle" style="font-size: 0.9rem;">
                                                <tbody>
                                                    <tr>
                                                        <td class="text-muted fw-semibold py-1" style="width: 45%;">Date / Time:</td>
                                                        <td class="text-dark fw-bold py-1">${r.rideDate} at ${r.rideTime}</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted fw-semibold py-1">Vehicle:</td>
                                                        <td class="text-dark py-1">${r.vehicleMake} ${r.vehicleModel} (${r.vehiclePlate})</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted fw-semibold py-1">Available Seats:</td>
                                                        <td class="text-dark py-1">${r.availableSeats} of ${r.totalSeats} seats</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted fw-semibold py-1">Price per Seat:</td>
                                    ...
                                    <div class="border-top pt-3 mt-3 d-flex flex-wrap gap-2 justify-content-between align-items-center">
                                        <a href="${pageContext.request.contextPath}/rides/details?id=${r.id}" class="btn btn-sm btn-outline-secondary px-3"><i class="bi bi-eye me-1"></i>View Details</a>
                                        <c:if test="${r.status == 'ACTIVE' || r.status == 'FULL'}">
                                            <div class="d-flex gap-2">
                                                <a href="${pageContext.request.contextPath}/rides/edit?id=${r.id}" class="btn btn-sm btn-outline-primary px-3"><i class="bi bi-pencil me-1"></i>Edit</a>
                                                <a href="${pageContext.request.contextPath}/rides/cancel?id=${r.id}" onclick="return confirm('Are you sure you want to cancel this ride?');" class="btn btn-sm btn-danger px-3">Cancel Ride</a>
                                            </div>
                                        </c:if>
                                    </div>
                                </div>
